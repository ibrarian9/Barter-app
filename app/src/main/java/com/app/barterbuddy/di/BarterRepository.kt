...
class BarterRepository private constructor(
    ...
    suspend fun getListChat(userId1: String, userId2: String): LiveData<ChatHistoryResponse> {
        val data = MutableLiveData<ChatHistoryResponse>()
        try {
            val res = apiService.getChatHistory(userId1, userId2)
            if (res.isSuccessful){
                data.postValue(res.body())
            }
        } catch (e: Exception){
            Log.d("error chat history", e.message.toString())
        }
        return data
    }
    ...
    suspend fun postChat(request: AddChat): LiveData<SuccessPost> {
        ...
        try {
            val res = apiService.postChat(request)
            if (res.isSuccessful){
                data.postValue(res.body())
            }
        } catch (e: Exception){
            Log.d("error chat", e.message.toString())
        }
        return data
    }
